...
namespace BigAmbitionsMP
{
    ...
    public static class BusinessSync
    {
        ...
        public static void TickClient()
        {
            ...
            try
            {
                ...
                foreach (var reg in gi.BuildingRegistrations)
                {
                    ...
                    var wire = info.ShallowCopy();                              // H-BIZFLAP-1: the remembered record stays intact; only the wire copy loses its logo bytes
                    StripUnchangedLogos(wire);                                  // attach-once applies to the wire copy only
                    MPClient.SendBusinessChange(wire);
        ...
        // process last attached them. The strip runs on the WIRE COPY only (H-BIZFLAP-1): the
        // record the sweeps remember keeps its list, so an attach/strip transition is not a
        // "change". An EMPTY list is safe everywhere: the receive side logs "no logo files
        // received" once and never wipes (GameStatePatcher ~:5215), and the host's adopt diff
        // skips empties. The JOIN snapshot bypasses this (BuildFullSnapshot → ReadInfo
