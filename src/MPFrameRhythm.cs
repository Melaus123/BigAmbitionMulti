...
namespace BigAmbitionsMP
{
    ...
    public static class MPFrameRhythm
    {
        ...
        /// <summary>Per-frame capture — call once per Update while in an MP session.</summary>
            ...
            try
            {
                ...
                if (ms >= SlowFrameMs)
                {
                    ...
                    if (_slowDetailCount <= DetailFirstN || _slowDetailCount % DetailEveryN == 0)
                        Plugin.Logger.LogInfo($"[Rhythm] slow frame #{_slowDetailCount}: {ms:F0}ms — beats this frame: {(matched.Length > 0 ? matched : "<none>")}");
                }
            ...
            try
            {
                ...
                if (oscillating && n > 0)
                {
                    ...
                    Plugin.Logger.LogWarning(
                        ...
                        + $"slow(≥{SlowFrameMs:F0}ms)={slow} period≈{period:F2}s gc={g0}/{g1}/{g2} | beat hits:{(beats.Length > 0 ? beats : " <none — cadence is not a registered beat>")}");
                }
            }
            catch { }
            finally
