...
namespace BigAmbitionsMP
{
    ...
    public class CompanyCandidatesPayload
    {
        ...
        // MERGER PHASE 4c PART 2a (D23): actions "claim-plan" / "release-plan" reuse this same table and this
        // same CandidateId field for a HEALTH-INSURANCE OFFER id. The id spaces do not collide (both are
        // uuids minted by the game) and one claim table is exactly what "one negotiation at a time in the
        // company" means. OwnerPid is the machine that raised the offer and is CHECKED at the host
        // (co-membership) instead of being looked up in the candidate pools, which do not list an offer.
    ...
    public class CompanyMessagePayload
    {
        ...
        public string CtxOfferId         { get; set; } = "";   // the health-insurance offer id - D23: the claim is keyed on it
        // MERGER PHASE 4c PART 2a (D23): what a RELAYED staff-insurance offer needs to be actionable on a
        // member. The offer object itself stays in the owner's save; these five rebuild a detached copy good
        // enough for the game's own negotiation dialog, and the ACCEPT/DECLINE route back as a plan edit.
        public string CtxPlanId          { get; set; } = "";   // the HR plan the offer is for
        public string CtxHqKey           { get; set; } = "";   // that HR plan's headquarters - where the edit routes
        public int    CtxPlanType        { get; set; }         // HealthInsurancePlanType
        public float  CtxOfferPrice      { get; set; }         // initialOfferPrice
        public float  CtxOfferMinPrice   { get; set; }         // minOfferPrice (the negotiator's floor)
    ...
    public class SharedWorkInfoPayload
    {
        ...
        public float                  Quote     { get; set; }             // wave 4 r2 (D21): Tab "sellquote" — the RUNNER's own recomputed sell-all total for this warehouse, the number the member's native confirmation then shows
        public string                 AckCode   { get; set; } = "";       // wave 4 r2 (minor f): Tab "mergerack" — a short CODE for a routed refusal ("dupe" / "shelf" / "funds" / "away" / "ranges"), never a text: the member maps it to the game's OWN notification key locally, so nothing on the wire can put words on a screen
    }
    ...
    public class SharedWorkEditPayload
    {
        public string PlayerId   { get; set; } = "";   // sender (validated SenderIs at the host)
        public string AddressKey { get; set; } = "";
        public string Op         { get; set; } = "";   // "driver" | "recipe" | "produce" | "order" | "alias" | v18 7b: "contract" (full editable state) | "endcontract" | merger phase-2 wave 4: "mergercontract" (create a wholesale delivery contract on the operator - StrValue = the wholesale store's address key) | "mergersellall" (sell a warehouse's whole inventory on the runner - D21: Estimate carries the QUOTE the runner itself gave, and the runner sells only if its own recompute still equals it) | "mergerplan" (replace-by-id one HQ logistics plan on the operator - Plan carries the whole plan) | MERGER 4c part 2a: "mergerplanedit" (ONE edit on one of the four NON-logistics HQ plan families, applied by the runner of the headquarters that owns the plan - Family/PlanId/PlanOp/EditSeq below say which, the matching DTO carries the plan as the member's screen has it, and StrValue/IntValue/BoolValue/Estimate carry the op's own arguments; PlanOp "refused" is the RUNNER'S ANSWER back to the one member that sent the edit - StrValue is the reason and StationId names that member, so the host FORWARDS it instead of routing or fanning it out) | H-MERGERHIRE-1: "mergercampaign" (book a RECRUITMENT campaign for a merger-flipped partner SHOP on that shop's REAL OWNER - AddressKey is the shop, AgencyKey the recruitment agency's address key, SkillName the skill asked for, IntValue the candidate count, BoolValue the full-time toggle, PartTime the part-time toggle, Days the deadline slider in days and Estimate the price the member's dialog showed; the owner clamps the two numbers to its own sliders, RECOMPUTES the price and charges its own figure)
        ...
        public float   Estimate  { get; set; }         // wave 4 (D21): "mergersellall" — the QUOTE the runner answered with. The runner recomputes at the commitment and sells ONLY if the two still match (0.005); otherwise it refuses and re-quotes, so the figure the player confirmed is the figure charged. H-MERGERHIRE-1: "mergercampaign" — the price the MEMBER's dialog showed. A campaign price is formula-only (50 x (maxDays - days + 1) x candidates, RecruitmentSettings.cs:87-94), so a difference is LOGGED on the owner and never refused: the owner's own figure is the one charged

        /// <summary>H-MERGERHIRE-1, ADDITIVE (protocol 24 has not shipped, so these are plain new members).
        /// "mergercampaign" only. BoolValue already carries the FULL-TIME toggle, so PartTime carries the other
        /// half of the schedule the member ticked; Days is the deadline slider, which both the price formula and
        /// the candidate find-times read (RecruitmentAgencyDialog.cs:192-198).</summary>
        ...
        // plus the whole plan as the member's screen holds it, addressed (AddressKey) at the HEADQUARTERS
        // that owns it. The host union-checks and RouteTargetFor()s it; the runner applies the op onto its
        // OWN native plan by id with the game's own method. Idempotent by (PlanId, EditSeq) at every step.
        public string Family   { get; set; } = "";     // "pricing" | "purchasing" | "hr" | "headhunter"
        public string PlanId   { get; set; } = "";     // the plan the edit is about ("add"/"create" mint it here)
        public string PlanOp   { get; set; } = "";     // see CompanyPlans.ApplyRouted for the op table
        public int    EditSeq  { get; set; }           // per plan id, monotonic on the member that sent it
        public PwPricingPlan?       PricingPlan    { get; set; }
        public PwImportPartnership? Partnership    { get; set; }
        public PwHrPlan?            HrPlan         { get; set; }
        public PwHeadhunterPlan?    HeadhunterPlan { get; set; }
    }
    ...
    public class MergerStatePayload
    {
        ...
        /// <summary>MERGER PHASE 3-B (additive): who simulates which absent member's addresses. Every
        /// machine needs it (P3-C's return leg, and any later UI); an older client ignores the field.</summary>
        public List<AbsenceInfo> Absences { get; set; } = new List<AbsenceInfo>();
    }
    ...
    public class TrafficCarDto
    {
        ...
        /// <summary>TRAFFIC-CONSIST T3 (2026-09-18, design Q4 P3): 1 = "stopped or braking" - the machine that
        /// RUNS this car saw one of Gley's stop-ish driving actions (GiveWay 15, StopInPoint 20, TempStop 60,
        /// StopInDistance 70, StopNow 90) or a speed below 0.5 m/s at sample time. The receiver never extrapolates
        /// such a car and never gives it a Hermite tangent, so a queue at a red light cannot be predicted into the
        /// car in front. Additive: default 0 is omitted from the JSON, an older sender sends none, and 0 means
        /// exactly what it meant before ("moving, or unknown").</summary>
        [Newtonsoft.Json.JsonProperty(DefaultValueHandling = Newtonsoft.Json.DefaultValueHandling.Ignore)]
    ...
    /// <summary>TRAFFIC-CONSIST T1 (2026-09-18): Client -> Host. The client's own leftover ambient cars during the
    /// switch-over to ghost mode, in the SAME rows the host's own stream uses (TrafficCarDto, unchanged), so the
    /// host's relay to the other peers is a copy and not a conversion. Unreliable lane; Seq is monotonic per client
    /// and the host drops any publish whose Seq is not greater than the last one it accepted.</summary>
    public class ClientTrafficSnapshotPayload
    {
        public List<TrafficCarDto> Cars { get; set; } = new();
        /// <summary>The PUBLISHER's unscaled clock at sample time. The host does not share that clock, so it
        /// stamps the stand-ins with its own arrival time; this rides along for diagnostics and for a future
        /// receiver that wants it.</summary>
        public float T { get; set; }
        public long  Seq { get; set; }
    }
    ...
    /// <summary>MERGER PHASE 3-B: ONE absence as the host holds it - the absent owner, the member
    /// designated to run their businesses, and exactly which addresses that covers.</summary>
    ...
    /// <summary>MERGER PHASE 3-B: the hand-over itself (Host -> simulator), and the simulator's ACK
    /// back on the same type (Ack set, everything else empty). PaperworkJson is the P3-A bundle as
    /// the host stored it - text, so a future bundle shape rides through an older host untouched.</summary>
    public class MergerHandoverPayload
    {
        public string OwnerPid      { get; set; } = "";
        public string OwnerStable   { get; set; } = "";
        public string SimulatorPid  { get; set; } = "";
        public int    SinceDay      { get; set; }
        public List<string> Addresses { get; set; } = new();
        public string PaperworkJson { get; set; } = "";
    ...
    /// <summary>MERGER PHASE 2 WAVE 4 (D18): one owner's agreement lists as DISPLAY COPIES for a
    /// co-member. Host -> member only. Action "clear" carries no rows and retires that owner's copies.</summary>
    public class CompanyListsPayload
    {
        public string PlayerId { get; set; } = "";     // always "host" on the wire
        public string OwnerPid { get; set; } = "";     // whose agreements these are
        public string Action   { get; set; } = "";     // "lists" | "clear"
        public List<string> Addresses { get; set; } = new();          // the owner's merged addresses the rows belong to
        public List<PwDeliveryContract> DeliveryContracts { get; set; } = new();
        public List<PwLogisticsPlan>    LogisticsManagerPlans { get; set; } = new();

        // MERGER PHASE 4c part 1 (D20-6): the four remaining HEADQUARTERS plan families ride the same
        // payload - no new MessageType and no second cadence. They are SCREEN-LAYER ONLY on the receiver
        // (CompanyPlans' registry): an installed HR or headhunter copy would train, insure and recruit a
        // second time off the replicated employee, which is outside the authority veil.
        public List<PwPricingPlan>       PricingManagerPlans { get; set; } = new();
        public List<PwImportPartnership> ImportPartnerships  { get; set; } = new();
        public List<PwHrPlan>            HrManagerPlans      { get; set; } = new();
        public List<PwHeadhunterPlan>    HeadhunterPlans     { get; set; } = new();
