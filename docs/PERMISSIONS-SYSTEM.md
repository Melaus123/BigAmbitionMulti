...
**TBD** — to design after vehicle permissions. Foundations seen in code: `WorldSnapshotPayload.BuildingOwners` (rents/operates) + `BuildingRealEstateOwners` (bought property), keyed by "street+number"; `InteriorSync` for interior state; currently **anyone can enter any building** (no access check). **[code]**
